...
def joynernacci(number):
    ...
    for index in range(2, number+1):
        ...
        if index // 2:
            current = two_previous[0] + two_previous[1]
            two_previous[0] = two_previous[1]
            two_previous[1] = current
        else:
            current = abs(two_previous[0] - two_previous[1])
            two_previous[0] = two_previous[1]
            two_previous[1] = current            
        return two_previous[1]

    for index in range(0, 200):
        return(joynernacci(index))
