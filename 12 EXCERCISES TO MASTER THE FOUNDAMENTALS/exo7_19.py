...
def check_horse_winner(string1,string2):
    player = []
    next_player = []
    indice = 0
    track_record = "HORSE"
    ...
    for player in players:
        for next_player in players:
            if track_record[indice] is track_record[4]:
                break
            else:
                if player is "M" and next_player is "M":
                    continue
                elif player is "M" and next_player is not "M":
                    player[indice] = next_player[indice] = track_record[indice]
    for player in players:
        if player == "HORSE" and next_player is not "HORSE":
            return "Player1 wins"
        elif player is not "HORSE" and next_player == "HORSE":
            return "Player2 wins"
        else:
            return "ALL WINNERS"
