...
def find_net_force(force_instances):
    ...
    for instances in force_instances:
        total_horizontal += float(instances.get_horizontal())
        total_vertical += float(instances.get_vertical())
        
    net_angle = round(degrees(atan2(total_vertical, total_horizontal)), 1)  
    
    net_force = round((total_horizontal ** 2 + total_vertical ** 2) ** 0.5, 1)
    
    totalForce = Force(net_force, net_angle)
