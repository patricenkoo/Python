...
def insertion(oneList):    
    for index in range(1,len(oneList)):
        temp = oneList[index]
        insertIndex = index - 1
        while insertIndex >= 0 and temp < oneList[insertIndex]:
            oneList[insertIndex + 1] = oneList[insertIndex]
            insertIndex -= 1
        oneList[insertIndex + 1] = temp
    return oneList
